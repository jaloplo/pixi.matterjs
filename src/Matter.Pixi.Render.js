...
(function() {
    ...
    Render.assets = function(render, resource) {
        Loader.add(resource).load(function(res) {
            Matter.Events.trigger(render, 'afterLoad', res);
        });
    };

    Render.sprite = function(render, resource, texture, options) {
        var sprite = new PIXI.Sprite(PIXI.loader.resources[resource].textures[texture]);
        sprite.x = options.x;
        sprite.y = options.y;
